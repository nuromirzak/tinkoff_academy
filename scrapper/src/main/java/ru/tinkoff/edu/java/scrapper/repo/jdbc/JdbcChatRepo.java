...
public class JdbcChatRepo implements ChatRepo {
    ...
    private static final String SQL_FIND_CHATS_BY_LIKE_LINK =
            "SELECT * FROM chat WHERE chat_id IN (SELECT chat_id FROM link_chat WHERE link_id IN (SELECT link_id FROM link WHERE url LIKE ?))";
